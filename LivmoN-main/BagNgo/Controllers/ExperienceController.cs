...
namespace BagNgo.Controllers
{
    ...
    public class ExperienceController : Controller
    {
        ...
        private readonly IExperienceDaysService experienceDaysService;
        ...
        public ExperienceController(ApplicationDbContext db, IPhotoService _phoroservice, IUserService _UserService, IExperiencesDatesServices _experiencesDatesServices , IExperienceDaysService _experienceDaysService, IFoodExperienceService _foodExperienceService , IActivityServices _activityServices,ILodgingExperienceService _lodgingExperienceService,ITransportExperienceServices _tranExpServ ,IExperienceService _experienceService, UserManager<Users> _userManager, SignInManager<Users> _signInManager, IHostService _hostService)
        {
            _db = db;
            ...
            experienceDaysService = _experienceDaysService;
        ...
        public async Task<IActionResult> CreateExperience( ExperienceViewModel model , string id)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                /*
                // Lodging Insert :
                if (experience.TransportExist == true)
                {
                    TransportExperience transport = new TransportExperience
                    {
                        ...
                        Description = model.DescriptionTransport,
                        ExperienceId = experience.ExperienceId,

                    };
                     await TransportExpService.InsertTransportExperience(transport);
                }

                if (experience.LodgingExist == true)
                {
                     LodgingExperience lodging = new LodgingExperience
                        {
                            Category = model.CategoryLodging,
                            Type = model.TypeLodging,
                            Adress = model.AdressLodging,
                            Description = model.DescriptionLodging,
                            Instructions = model.InstructionsLodging,
                            Criteria = model.CriteriaLodging,
                            StartDate = model.StartDateLodging,
                            EndDate = model.EndDateLodging,
                            ExperienceId = experience.ExperienceId,

                        };
                        await lodgingExperienceService.InsertLodgingExperience(lodging);
                }

                if (experience.FoodExist == true)
                {
                    FoodExperience food = new FoodExperience
                    {
                        NameDish = model.NameDish,
                        Description = model.DescriptionFood,
                        ExperienceId = experience.ExperienceId,

                    };
                     await foodExperienceService.InsertFoodExperience(food);

                }

                if (experience.DatType.Equals("Specific Date"))
                {
                    ExperienceDates date = new ExperienceDates
                    {
                        EndTime = model.EndTimeSpecific,
                        StartTime = model.StartTimeSpecific,
                        ExperienceId = experience.ExperienceId,

                    };
                    await experiencesDatesServices.InsertExperienceDates(date);
                    ...
                    await experienceDaysService.InsertDays(days);

                }


                return Ok(activites.Count);*/
                return Ok(result);
        ...
        [HttpGet("GetAllActiveValidExperiences")]
