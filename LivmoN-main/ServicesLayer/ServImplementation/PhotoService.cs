        ...
        public async Task<ImageUploadResult> AddCADTouristFilesAsync(IFormFile file)
        {
            ...
            if (file.Length > 0)
            {
                ...
                var uploadParams = new ImageUploadParams
                {
                    File = new FileDescription(file.FileName, stream),
                    //   Transformation = new Transformation().Height(500).Width(500).Crop("fill").Gravity("face"),
                    Folder = "Clients CAD Tourist Files Nour",
                };
                uploadResult = await _cloudinary.UploadAsync(uploadParams);
            }
        ...
        public async Task<ImageUploadResult> AddLicenceFilesAsync(IFormFile file)
        {
            ...
            if (file.Length > 0)
            {
                ...
                var uploadParams = new ImageUploadParams
                {
                    File = new FileDescription(file.FileName, stream),
                    //   Transformation = new Transformation().Height(500).Width(500).Crop("fill").Gravity("face"),
                    Folder = "Clients Licence Files Nour",
                };
                uploadResult = await _cloudinary.UploadAsync(uploadParams);
            }
        ...
        public async Task<ImageUploadResult> AddPhotoAsync(IFormFile file)
        {
            ...
            if (file.Length > 0)
            {
                ...
                var uploadParams = new ImageUploadParams
                {
                    File = new FileDescription(file.FileName, stream),
                 //   Transformation = new Transformation().Height(500).Width(500).Crop("fill").Gravity("face"),
                    Folder = "Client Profiles Pictures Nour",
                };
                uploadResult = await _cloudinary.UploadAsync(uploadParams);
            }
